function CanvasSand4() : SimulationCore() constructor {
	
	var _w = display_get_height()
	var _h = display_get_height()
	grid = array_create(_w);
	var _i=0; repeat (_w) {
		grid[_i] = array_create(_h);
	_i++}
	
	// === Override STEP ===
	static step = function() {
		var _w = array_length(grid);
		var _h = array_length(grid);
		
		for (var _x = 0; _x < _w; _x++) {
			
			var _col = grid[@ _x];
			var _col_left = (_x-1 >= 0) ? grid[@ _x-1] : undefined;
			var _col_right = (_x+1 < _w) ? grid[@ _x+1] : undefined;
