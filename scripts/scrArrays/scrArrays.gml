...
function array_create_2d(w, h, v=undefined) {
	...
	var _arr = array_create(w, 0);
	for (var i = 0; i < w; i++) {
		_arr[i] = array_create(h, v)
	}
	return _arr
}
...
function array_slice(_array, _index, _length) {
	var _new_arr = [];
	array_copy(_new_arr, 0, _array, _index, _length)
	_array = _new_arr;
	return _array;
}